// Node-RED Function body, two outputs: measurement requests / watchdog status.
...
try {
    ...
    // Capture the SOC once; both energy requests carry this same snapshot.
    msg._eff = {
        id: `${now}-${seq}`, ts: now,
    ...
    msg.payload = null;
    const completed = flow.get("batt_eff_last_completed_v3", "memoryOnly") || boot;
    let warning = null;
    if (now - completed > 15000) {
        ...
        node.status({fill: "blue", shape: "dot", text: "Requesting paired daily counters"});
    }
    return [msg, warning];
} catch (err) {
