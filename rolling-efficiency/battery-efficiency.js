/*
 * Rolling 168-hour SOC-adjusted battery efficiency, revision 4.1.
...
let state;
// Optional additive state extension: old v4 energy buffers need no migration.
function audit() {
...
try {
    ...
    if (prev) {
        ...
        if (state.blocked || dt>CFG.maxIntervalMs) {
            ...
            // Preserve the original failure cause when recovery closes its gap.
